...
import { TaskRunner, TaskError, Task } from '../TasksRunner';
...
import { CommandOptions, Fabrics, TaskArgs, PublishBackupData } from '../publish-packages/types';

const { cyan, magenta, reset } = chalk;
...
async function main(packageNames: string[], options: CommandOptions): Promise<void> {
  ...
  const taskRunner = new TaskRunner<[Fabrics, CommandOptions], PublishBackupData>({
    tasks,
    ...
    createBackupData(task, fabrics, options): PublishBackupData {
      const data = {
     ...
     */
    restoreBackup(backup, fabrics): void {
  ...
  try {
    await taskRunner.runAsync([], options);
    process.exit(0);
  } catch (error) {
    logger.error();

    if (error instanceof TaskError) {
      logger.error(`💥 Command failed at phase ${cyan(error.task.name)}.`);
    }
